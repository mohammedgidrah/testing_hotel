    ...
    axios
      ...
      .then((response) => {
        if (filterDate == "all") {
          ...
          let felteredExpenses = response.data.filter((item) => {
            return (
              new Date(item.expense_date).getFullYear() ==
          ...
          let felteredExpenses = response.data.filter((item) => {
            return (
              new Date(item.expense_date).getMonth() == new Date().getMonth()
            );
          });
          felteredExpenses.forEach((item) => {
            total_Expenses += JSON.parse(item.amount);
    ...
    axios
      ...
      .then((response) => {
        if (filterDate == "all") {
          ...
          let felteredPayments = response.data.filter((item) => {
            return (
              new Date(item.payment_date).getMonth() == new Date().getMonth()
            );
          });
          felteredPayments.forEach((item) => {
            totalIncome += JSON.parse(item.amount_paid);
      ...
      .then((response) => {
        if (filterDate == "all") {
          ...
          let felteredBookings = response.data.filter((item) => {
            return (
              new Date(item.check_in_date).toDateString() ==
              new Date().toDateString()
            );
          });
          ...
          let felteredBookings = response.data.filter((item) => {
            return (
              new Date(item.check_in_date).getFullYear() ==
              new Date().getFullYear()
            );
          });
          setTotalBookings(felteredBookings.length);
          ...
          let felteredBookings = response.data.filter((item) => {
            return (
              new Date(item.check_in_date).getMonth() == new Date().getMonth()
            );
          });
          setTotalBookings(felteredBookings.length);
          setLoading(false);
              ...
              <form className="mb-8 ">
                <div className="mt-1 flex nowrap justify-between">
                  ...
                  <select
                    ...
                    className="self-end rounded-md w-1/4 border border-gray-600 bg-inherit focus:border-indigo-500 focus:ring-indigo-500 text-gray-200"
                  >
                    <option value={t("allTime")} className="bg-gray-800">
                      {t("allTime")}{" "}
                    </option>
                    <option value={t("forThisYear")} className="bg-gray-800">
                      {t("forThisYear")}{" "}
                    </option>
                    <option value={t("forThisMonth")} className="bg-gray-800">
                      {t("forThisMonth")}{" "}
                    </option>
                    <option value={t("forToday")} className="bg-gray-800">
                      {t("forToday")}{" "}
                    </option>
                  </select>
                </div>
